...
for lesion_image in lesion_images:
    ...
    med_img[:,:,0] = median(image[:,:,0], disk(disk_size))
    med_img[:,:,1] = median(image[:,:,1], disk(disk_size))
    med_img[:,:,2] = median(image[:,:,2], disk(disk_size))
    ...
    ax.set_title('median with window size {0}'.format(disk_size), fontsize=20)
    ax.imshow(med_img)

    med2_img[:,:,0] = median(image[:,:,0], disk(disk_size*3))
    med2_img[:,:,1] = median(image[:,:,1], disk(disk_size*3))
    med2_img[:,:,2] = median(image[:,:,2], disk(disk_size*3))

    ax = fig.add_subplot(133)
    ax.set_title('median with window size {0}'.format(disk_size+3), fontsize=20)
